            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div class="lg:col-span-2 space-y-8">
                        ...
                        <div class="flex flex-col sm:flex-row items-start gap-6">
                            ...
                            <div class="flex-grow">
                                <div class="flex items-center gap-2 mb-1">
                                    <h1 class="font-display text-2xl md:text-3xl font-bold text-white">{{ $freelancer->user->name }}</h1>
                                </div>
                                <p class="text-primary-400 font-medium mb-1">{{ $freelancer->title }}</p>
                                <p class="text-dark-400 text-sm mb-3">{{ $freelancer->category->name }} • {{ $freelancer->specialization }}</p>
                                <div class="flex flex-wrap items-center gap-4 text-sm">
                                    <div class="flex items-center gap-1">
                                        <svg class="w-4 h-4 text-yellow-400" fill="currentColor" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/></svg>
                                        <span class="text-white font-semibold">{{ number_format($freelancer->rating_avg, 1) }}</span>
                                        <span class="text-dark-400">({{ $freelancer->rating_count }} reviews)</span>
                                    </div>
                                    <span class="badge bg-blue-500/20 text-blue-400 border border-blue-500/30">{{ $freelancer->experience_years }} years experience</span>
                                    @if($freelancer->is_mobile)
                                        <span class="badge bg-emerald-500/20 text-emerald-400 border border-emerald-500/30">🏠 Travels to you ({{ $freelancer->service_radius_km }}km)</span>
                                    @endif
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="glass-card p-6">
                        ...
                        <p class="text-dark-300 leading-relaxed">{{ $freelancer->bio }}</p>
                    </div>
                    ...
                    {{-- Reviews --}}
                    <div class="glass-card p-6">
                        <h2 class="font-display font-semibold text-xl text-white mb-6">Reviews ({{ $freelancer->rating_count }})</h2>
                        <div class="space-y-4">
                ...
                <div class="space-y-6">
                    <div class="glass-card p-6 sticky top-24">
                        ...
                        <div class="mb-6">
                            <h4 class="text-white font-medium mb-3 text-sm">Availability</h4>
                            <div class="space-y-2 text-sm">
                                @foreach($freelancer->timeSlots->sortBy('day_of_week') as $slot)
                                    <div class="flex justify-between text-dark-300">
                                        ...
                                        <span class="text-emerald-400">{{ substr($slot->open_time, 0, 5) }} - {{ substr($slot->close_time, 0, 5) }}</span>
                                    </div>
                                @endforeach
                            </div>
                        </div>
                        ...
                        <a href="{{ route('bookings.create.freelancer', $freelancer->id) }}" class="btn-secondary w-full !py-3 text-sm">Request Home Service 🏠</a>
                    </div>
                </div>
            </div>
        </div>
