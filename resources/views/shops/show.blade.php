<x-layouts.app :title="$shop->name">
    {{-- Cover --}}
    <section class="relative h-72 md:h-96 bg-gradient-to-br from-primary-100 via-primary-50 to-gray-50 overflow-hidden">
        <div class="absolute inset-0 flex items-center justify-center opacity-10"><svg class="w-48 h-48 text-primary-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1" d="M9.813 15.904L9 18.75l-.813-2.846a4.5 4.5 0 00-3.09-3.09L2.25 12l2.846-.813a4.5 4.5 0 003.09-3.09L9 5.25l.813 2.846a4.5 4.5 0 003.09 3.09L15.75 12l-2.846.813a4.5 4.5 0 00-3.09 3.09zM18.259 8.715L18 9.75l-.259-1.035a3.375 3.375 0 00-2.455-2.456L14.25 6l1.036-.259a3.375 3.375 0 002.455-2.456L18 2.25l.259 1.035a3.375 3.375 0 002.455 2.456L21.75 6l-1.036.259a3.375 3.375 0 00-2.455 2.456z"/></svg></div>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 -mt-20 relative z-10 pb-20">
            ...
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <div class="flex items-center gap-2 mb-2">
                        <span class="badge bg-primary-50 text-primary-700 border border-primary-200">{{ $shop->category->name }}</span>
                        @if($shop->is_featured)<span class="badge bg-yellow-50 text-yellow-700 border border-yellow-200"><svg class="w-3.5 h-3.5 mr-1 text-amber-500" fill="currentColor" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/></svg>Featured</span>@endif
                    </div>
                    <h1 class="font-display text-3xl md:text-4xl font-bold text-gray-900 mb-2">{{ $shop->name }}</h1>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div class="lg:col-span-2 space-y-8">
                        ...
                        @foreach($shop->services as $service)
                            <div class="flex items-center justify-between p-4 rounded-xl bg-gray-50 hover:bg-gray-100 transition-colors">
                                <div class="flex-grow">
                                    ...
                                    <span class="text-gray-400 text-xs mt-1 inline-block">⏱ {{ $service->formatted_duration }}</span>
                                </div>
                ...
                <div class="glass-card p-6">
                    <h2 class="font-display font-semibold text-xl text-gray-900 mb-6">Our Team</h2>
                    <div class="grid grid-cols-2 sm:grid-cols-3 gap-4">
                        @foreach($shop->staffMembers as $staff)
                            <div class="text-center p-4 rounded-xl bg-gray-50">
                                ...
                                <h4 class="text-gray-900 font-medium text-sm">{{ $staff->name }}</h4>
                                <p class="text-gray-500 text-xs">{{ $staff->title }}</p>
                            </div>
                        @endforeach
                    </div>
                </div>
                ...
                {{-- Reviews --}}
                <div class="glass-card p-6">
                    <h2 class="font-display font-semibold text-xl text-gray-900 mb-6">Reviews ({{ $shop->rating_count }})</h2>
                    <div class="space-y-4">
            ...
            <div class="space-y-6">
                <div class="glass-card p-6 sticky top-24">
                    <h3 class="font-display font-semibold text-lg text-gray-900 mb-4">Quick Info</h3>
                    <div class="space-y-4 text-sm">
                        ...
                        <div class="flex items-start gap-3 text-gray-600">
                            <svg class="w-5 h-5 text-primary-600 shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"/></svg>
                            {{ $shop->address }}
                        </div>
                        <div class="flex items-center gap-3 text-gray-600">
                    ...
                    <div class="mt-6 pt-6 border-t border-gray-200">
                        <h4 class="text-gray-900 font-medium mb-3">Opening Hours</h4>
                        <div class="space-y-2 text-sm">
                            @foreach($shop->timeSlots->sortBy('day_of_week') as $slot)
                                <div class="flex justify-between text-gray-600">
                                    <span>{{ $slot->day_name }}</span>
                                    <span class="{{ $slot->is_available ? 'text-emerald-600' : 'text-gray-400' }}">
