<x-layouts.dashboard title="Shop Dashboard">
    <x-slot:sidebar>
        <a href="{{ route('shop.dashboard') }}" class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm {{ request()->routeIs('shop.dashboard') ? 'bg-primary-600/20 text-primary-300' : 'text-dark-300 hover:text-white hover:bg-white/5' }} transition-colors">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6"/></svg>
            Overview
        </a>
        <a href="{{ route('shop.bookings') }}" class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm {{ request()->routeIs('shop.bookings') ? 'bg-primary-600/20 text-primary-300' : 'text-dark-300 hover:text-white hover:bg-white/5' }} transition-colors">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
            Bookings
        </a>
        <a href="{{ route('shop.services') }}" class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm {{ request()->routeIs('shop.services') ? 'bg-primary-600/20 text-primary-300' : 'text-dark-300 hover:text-white hover:bg-white/5' }} transition-colors">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10"/></svg>
            Services
        </a>
        <a href="{{ route('shops.show', $shop->slug) }}" class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm text-dark-300 hover:text-white hover:bg-white/5 transition-colors">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        ...
        <div class="glass-card p-5">
            <p class="text-dark-400 text-sm mb-1">Total Reviews</p>
            <p class="text-2xl font-display font-bold text-white">{{ $totalReviews }} <span class="text-sm text-yellow-400">⭐ {{ number_format($shop->rating_avg, 1) }}</span></p>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div class="glass-card p-6">
            <div class="flex justify-between items-center mb-4">
                ...
                <a href="{{ route('shop.bookings') }}" class="text-primary-400 text-sm hover:underline">View all</a>
            </div>
            <div class="space-y-3">
                @forelse($shop->bookings->take(5) as $booking)
                    <div class="flex items-center justify-between p-3 rounded-xl bg-white/5">
                        <div>
                            ...
                            <p class="text-dark-400 text-xs">{{ $booking->booking_date->format('M d') }} at {{ \Carbon\Carbon::createFromFormat('H:i:s', $booking->start_time)->format('g:i A') }}</p>
                        </div>
            ...
            <div class="space-y-3">
                @forelse($shop->reviews->take(5) as $review)
                    <div class="p-3 rounded-xl bg-white/5">
                        ...
                        <p class="text-dark-300 text-sm">{{ Str::limit($review->comment, 100) }}</p>
                    </div>
                @empty
