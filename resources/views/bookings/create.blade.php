        ...
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8" x-data="bookingForm()">
            ...
            <div class="flex items-center justify-center gap-2 mb-10">
                @foreach(['Services', 'Date & Time', 'Details'] as $i => $stepName)
                    <div class="flex items-center gap-2">
                        <div :class="step > {{ $i }} ? 'bg-primary-600 text-white' : (step === {{ $i }} ? 'bg-primary-50 text-primary-700 border-primary-500' : 'bg-gray-100 text-gray-400 border-gray-200')"
                             class="w-8 h-8 rounded-full flex items-center justify-center text-sm font-semibold border transition-all">{{ $i + 1 }}</div>
                        <span :class="step >= {{ $i }} ? 'text-gray-900' : 'text-gray-400'" class="text-sm hidden sm:inline">{{ $stepName }}</span>
                        @if($i < 2)<div class="w-8 h-px bg-gray-200 mx-1"></div>@endif
                    </div>
                @endforeach
            </div>
            ...
            <form method="POST" action="{{ route('bookings.store') }}">
                ...
                <div x-show="step === 0" x-transition class="glass-card p-6 md:p-8">
                    <h2 class="font-display font-semibold text-xl text-gray-900 mb-6">Select Services</h2>
                    <div class="space-y-3">
                        @foreach($services as $service)
                            <label class="flex items-center justify-between p-4 rounded-xl bg-gray-50 hover:bg-gray-100 cursor-pointer transition-colors border border-transparent"
                                   :class="selectedServices.includes({{ $service->id }}) ? 'border-primary-500 bg-primary-50' : ''">
                                <div class="flex items-center gap-3">
                                    <input type="checkbox" name="services[]" value="{{ $service->id }}"
                                           x-model.number="selectedServices" :value="{{ $service->id }}"
                                           @change="updateTotal()" class="rounded border-gray-300 bg-white text-primary-500 focus:ring-primary-500">
                                    <div>
                                ...
                                <span class="text-gray-900 font-semibold shrink-0 ml-4">${{ number_format($service->price, 0) }}</span>
                            </label>
                        @endforeach
                    </div>
                    <div class="mt-6 flex items-center justify-between">
                        <div>
                            ...
                            <span class="text-gray-900 font-display font-bold text-xl ml-2">$<span x-text="total"></span></span>
                        </div>
                ...
                <div x-show="step === 1" x-transition class="glass-card p-6 md:p-8">
                    <h2 class="font-display font-semibold text-xl text-gray-900 mb-6">Choose Date & Time</h2>
                    <div class="mb-6">
                        <label class="text-gray-700 text-sm mb-2 block">Select Date</label>
                    ...
                    <div x-show="bookingDate">
                        <label class="text-gray-700 text-sm mb-3 block">Available Time Slots</label>
                        <div x-show="loadingSlots" class="text-gray-500 text-sm py-4">Loading available slots...</div>
                        <div x-show="!loadingSlots" class="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 gap-2">
                            <template x-for="slot in availableSlots" :key="slot.time">
                                <button type="button" @click="if(slot.available) selectedTime = slot.time"
                                        :class="slot.available ? (selectedTime === slot.time ? 'bg-primary-600 text-white border-primary-500' : 'bg-gray-50 text-gray-900 hover:bg-gray-100 border-gray-200') : 'bg-gray-50 text-gray-300 cursor-not-allowed border-gray-100 line-through'"
                        ...
                        <div x-show="!loadingSlots && availableSlots.length === 0" class="text-gray-500 text-sm py-4">No slots available for this date.</div>
                        <input type="hidden" name="start_time" x-model="selectedTime">
                    </div>

                    @if($staff->count())
                    <div class="mt-6">
                        <label class="text-gray-700 text-sm mb-3 block">Preferred Staff (Optional)</label>
                        <div class="grid grid-cols-2 sm:grid-cols-3 gap-3">
                            <label class="flex items-center gap-3 p-3 rounded-xl bg-gray-50 cursor-pointer border border-transparent hover:bg-gray-100"
                                   :class="!staffId ? 'border-primary-500 bg-primary-50' : ''">
                                <input type="radio" name="staff_member_id" value="" x-model="staffId" class="text-primary-500 focus:ring-primary-500">
                                <span class="text-gray-600 text-sm">Any available</span>
                            </label>
                            @foreach($staff as $member)
                                <label class="flex items-center gap-3 p-3 rounded-xl bg-gray-50 cursor-pointer border border-transparent hover:bg-gray-100"
                                       :class="staffId == '{{ $member->id }}' ? 'border-primary-500 bg-primary-50' : ''">
                                    <input type="radio" name="staff_member_id" value="{{ $member->id }}" x-model="staffId" class="text-primary-500 focus:ring-primary-500">
                                    <div>
                                        <span class="text-gray-900 text-sm">{{ $member->name }}</span>
                                        <span class="text-gray-500 text-xs block">{{ $member->title }}</span>
                                    </div>
                                </label>
                            @endforeach
                        </div>
                ...
                <div x-show="step === 2" x-transition class="glass-card p-6 md:p-8">
                    <h2 class="font-display font-semibold text-xl text-gray-900 mb-6">Confirm Details</h2>

                    @if($providerType === 'freelancer')
                    <div class="mb-4">
                        <label class="text-gray-700 text-sm mb-2 block">Your Address (for home service)</label>
                        <input type="text" name="customer_address" class="input-dark" placeholder="Enter your address">
                    </div>
                    @endif
                    <div class="mb-4">
                        <label class="text-gray-700 text-sm mb-2 block">Phone Number</label>
                        <input type="text" name="customer_phone" class="input-dark" placeholder="Your phone number">
                    </div>
                    <div class="mb-6">
                        <label class="text-gray-700 text-sm mb-2 block">Notes (optional)</label>
                    ...
                    <div class="p-4 rounded-xl bg-gray-50 mb-6">
                        <h3 class="text-gray-900 font-semibold mb-3">Booking Summary</h3>
                        <div class="space-y-2 text-sm">
                            <div class="flex justify-between text-gray-600">
                                <span>Date</span><span class="text-gray-900" x-text="bookingDate"></span>
                            </div>
                            <div class="flex justify-between text-gray-600">
                                <span>Time</span><span class="text-gray-900" x-text="selectedTime"></span>
                            </div>
                            <div class="border-t border-gray-200 my-2 pt-2 flex justify-between">
                                <span class="text-gray-900 font-semibold">Total</span>
                                <span class="text-primary-600 font-display font-bold text-lg">$<span x-text="total"></span></span>
                            </div>
                        </div>
                    </div>
